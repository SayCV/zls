...
pub const Type = union(TypeKind) {
    ...
    pub fn jsonParseFromValue(allocator: std.mem.Allocator, source: std.json.Value, options: std.json.ParseOptions) std.json.ParseFromValueError!Type {
        ...
        inline for (comptime std.meta.fieldNames(Type), comptime std.meta.fieldTypes(Type)) |field_name, field_type| {
            if (std.mem.eql(u8, kind.string, field_name)) {
                return @unionInit(Type, field_name, try std.json.parseFromValueLeaky(field_type, allocator, source, options));
            }
        }
        return error.UnexpectedToken;
    }
