...
pub const JsonRPCMessage = union(enum) {
    ...
    pub fn jsonParseFromValue(
        ...
        for (source.object.keys(), source.object.values()) |field_name, field_source| {
            inline for (comptime std.meta.fieldNames(Fields)) |sub_field_name| {
                const field_enum = comptime @field(std.meta.FieldEnum(Fields), sub_field_name);
                if (std.mem.eql(u8, sub_field_name, field_name)) {
                    @field(fields, sub_field_name) = try Fields.parseFromValue(field_enum, allocator, field_source, options);
                    break;
                }
            } else {
